...
- **PATH checkbox:** Installer includes an optional checkbox "Add Heka to PATH" which appends the install directory to the user's PATH environment variable. Default: checked. Implementation: NSIS `EnvVar::SetUserPath` or manual registry edit for `HKCU\Environment\Path`.
...
6. Installer smoke: `make package` runs end-to-end on this machine (verifies NSIS toolchain presence), output exists.
7. PATH: installer with PATH checkbox checked modifies user PATH; `heka` command works from new terminal.
8. `make check` green.
...
4. `make package` produces a per-user installer with PATH option; installed app opens and reports the right version (manual with user: install → launch → version check).
5. PATH checkbox works: after install with checkbox, `heka` is available in new terminal sessions.
