    ...
    <button
      ...
      className={`inline-flex size-6 shrink-0 items-center justify-center rounded-md border outline-none transition-all ${
        copied
          ...
          : 'border-transparent text-foreground/40 hover:border-foreground/25 hover:text-accent'
      }`}
    >
      {copied ? (
...
export function AboutPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      {/* Hero + How It Works */}
      <section className="overflow-hidden rounded-2xl border border-border/80 bg-surface/70 shadow-sm backdrop-blur-sm">
        <div className="flex flex-wrap items-center gap-4 p-5">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <h2 className="text-lg font-bold tracking-tight text-foreground">Heka</h2>
        ...
        <div className="grid border-t border-border/60 sm:grid-cols-3">
          {HOW_IT_WORKS.map((item, i) => (
            ...
            >
              <div className={`mt-0.5 shrink-0 ${i === 1 ? 'text-accent' : 'text-foreground/50'}`}>
                {item.icon}
              </div>
              <div className="min-w-0">
                <div className="text-xs font-semibold text-foreground">{item.title}</div>
                <p className="mt-0.5 text-[11px] leading-relaxed text-foreground/55">{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid items-start gap-4 lg:grid-cols-2">
        ...
        <section className="space-y-2">
          <div className="px-1">
            <h3 className="text-sm font-semibold text-foreground/75">Security</h3>
          </div>
          <div className="rounded-2xl border border-border/80 bg-surface/70 p-2 shadow-sm backdrop-blur-sm">
            <ul>
              {SECURITY_FEATURES.map((item) => (
                <li
                  key={item}
                  className="flex items-start gap-2.5 rounded-lg px-2.5 py-1.5 transition-colors hover:bg-surface-secondary/50"
                >
                  <svg className="mt-0.5 size-3.5 shrink-0 text-emerald-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                    <polyline points="20 6 9 17 4 12" />
                  </svg>
                  <span className="text-xs leading-relaxed text-foreground/75">{item}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        <section className="space-y-2">
          <div className="flex items-center justify-between gap-2 px-1">
            <h3 className="text-sm font-semibold text-foreground/75">Agent Integration</h3>
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={handleCopySkill}
                className="inline-flex items-center gap-1.5 rounded-full border border-border/80 bg-surface/80 px-3 py-1 text-xs font-medium text-foreground/75 shadow-sm transition-colors hover:border-accent hover:text-accent dark:hover:border-accent dark:hover:text-accent"
              >
                {skillCopied ? (
                  <>
          ...
          <div className="overflow-hidden rounded-2xl border border-border/80 bg-surface/70 shadow-sm backdrop-blur-sm">
            <div className="flex items-center gap-1.5 border-b border-border/60 px-3.5 py-2.5">
              ...
              <span className="size-2 rounded-full bg-foreground/30" />
              <span className="size-2 rounded-full bg-foreground/30" />
              <span className="ml-2 font-mono text-[10px] text-foreground/50">heka-skill.md</span>
            </div>
            <pre className="overflow-auto p-3.5 font-mono text-[11px] leading-relaxed text-foreground/75">
...
$ heka daemon status             # Verify daemon`}
          ...
          <p className="px-1 text-[11px] leading-relaxed text-foreground/55">
            Copy or download the skill file into your AI assistant's skills directory. It teaches the agent every
            command, the{' '}
            <code className="rounded bg-surface-secondary px-1 py-0.5 font-mono text-[10px]">--json</code>{' '}
      ...
      <section className="space-y-2">
        <div className="flex items-baseline justify-between gap-2 px-1">
          <h3 className="text-sm font-semibold text-foreground/75">CLI Reference</h3>
        ...
        <div className="grid items-start gap-4 md:grid-cols-2">
          {CLI_COMMAND_COLUMNS.map((columns, ci) => (
            <div
              key={ci}
              className="overflow-hidden rounded-2xl border border-border/80 bg-surface/70 shadow-sm backdrop-blur-sm"
            >
              {columns.map((section) => (
                <div key={section.title} className="border-b border-border/60 p-3 last:border-0">
                  ...
                  {section.commands.map((cmd) => (
                    <div
                      key={cmd.cmd}
                      className="group flex items-center justify-between gap-3 rounded-lg px-1.5 py-1 transition-colors hover:bg-surface-secondary/50"
                    >
                      <code className="truncate font-mono text-xs text-foreground">{cmd.cmd}</code>
                      <span className="flex min-w-0 items-center gap-1.5">
                        <span className="truncate text-right text-[11px] text-foreground/50">{cmd.desc}</span>
      ...
      {/* Footer */}
      <div className="border-t border-border/80 pt-4 text-center text-[11px] text-foreground/50">
        Built with ❤️ by{' '}
        <button
          type="button"
