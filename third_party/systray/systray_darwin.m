...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
  ...
  // Primary anti-Dock mechanism for bundled launches is LSUIElement=1 in
  // the app's Info.plist: the daemon process is a UIElement from birth and
  // never gets a Dock tile (the GUI re-asserts .regular itself — Wails'
  // AppDelegate forces NSApplicationActivationPolicyRegular at startup).
  // This switch is the fallback for bare (non-bundled) dev runs where the
  // plist cannot apply; it is a harmless no-op under LSUIElement.
  // macOS 26 Tahoe note: status items created only with a title (no image)
  // while .accessory were observed not to render; Heka always sets an icon
  // first (see internal/osapp/tray.go), so item creation here is safe.
  [NSApp setActivationPolicy:NSApplicationActivationPolicyAccessory];
  // Opt out of aggressive termination so the daemon is not killed while
  // idle in the background.
  [[NSProcessInfo processInfo] disableSuddenTermination];
  [[NSProcessInfo processInfo] disableAutomaticTermination:@"systray"];
  systray_ready();
}
...
- (void)setIcon:(NSImage *)image {
  ...
  if (!policySwitched) {
    policySwitched = YES;
    // Belt and braces: applicationDidFinishLaunching already dropped us to
    // .accessory (before any Go callback — see there for why). Re-assert
    // after the runloop settles, so a lost first switch can never leave
    // the daemon in the Dock / app switcher.
